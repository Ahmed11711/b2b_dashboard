...
export default function PackagesPage() {
  ...
  const loadData = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await getAll(packagesEndpoint, cleanParams);
  ...
  const handleCreate = async (formData) => {
    try {
      ...
      await createItem(packagesEndpoint, payload);
      setShowCreate(false);
      loadData(1);
      return { success: true };
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    await deleteItem(packagesEndpoint, deleteRow.id);
    setDeleteRow(null);
      ...
      <div className="max-w-[1600px] mx-auto px-4 space-y-6">
        ...
        <div className="relative">
          ...
          <TableComponent
            ...
            onEdit={(row) => navigate(`/Packages/edit/${row.id}`)}
            onView={(row) => navigate(`/Packages/view/${row.id}`)}
